...
@Injectable()
export class GetCards_GODS {
  async getCardsGODS(cardsETH, cards_GODS_New, priceUSD, cardsGODS, priceGODS) {
    try {
      let cards = [];

      cards_GODS_New &&
        cardsGODS.map((item) => {
          let card = cards_GODS_New.find(
            (el) => el.name === item.name && el.quality === item.quality,
          );

          if (card === undefined) return;
          const old_price = Number(item.price) * Number(priceGODS);
          const new_price = Number(card.price) * Number(priceGODS);
